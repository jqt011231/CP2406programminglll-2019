class Test_Simulator {
    ...
    public static void main(String[] args) throws InterruptedException,NullPointerException {
        ...
        vehicle.current_park_car=Road1;
        road=vehicle.current_park_car;

        while(finish == false) {
            light.lightStatus();
            System.out.println(vehicle.name + " at " + vehicle.current_park_car.name
                    + vehicle.position + " Traffic Light Condition: " + light.color);
            vehicle.movement();
            road.position_info(vehicle);
            if (road.traffic_meet == true) {

                if(road.road_between == null) {
                    finish = true;
                }
